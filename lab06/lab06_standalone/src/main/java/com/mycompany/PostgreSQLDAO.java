...
public class PostgreSQLDAO {
    ...
    private final String SELECT_BY_ID = "SELECT * FROM persons WHERE id = ?::integer";
    ...
    void getPersonById(Integer id) throws NotFoundException {
        try (Connection connection = ConnectionUtil.getConnection()) {
            PreparedStatement preparedStmt = connection.prepareStatement(SELECT_BY_ID);
            preparedStmt.setInt(1, id);
            ResultSet rs = preparedStmt.executeQuery();
            if (!rs.next()) {
                throw new NotFoundException("NotFoundException: person (" + id.toString() + ") is not found");
            }
        } catch (SQLException ex) {
            Logger.getLogger(PostgreSQLDAO.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public String updatePerson(Integer id, Person updatedPerson) throws NotFoundException {
        getPersonById(id);
        try (Connection connection = ConnectionUtil.getConnection()) {
    ...
    public String deletePerson(Integer id) throws NotFoundException {
        getPersonById(id);
        try (Connection connection = ConnectionUtil.getConnection()) {
